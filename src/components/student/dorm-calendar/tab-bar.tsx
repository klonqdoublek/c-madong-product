...
export function TabBar({ active, onChange, pendingCount }: TabBarProps) {
  return (
    <div className="flex border-b border-black/8 bg-[#fbf6e9] px-4">
      {TABS.map(({ key, label }) => {
        ...
        return (
          <button
            ...
            className={cn(
              ...
              isActive ? "text-[#52ad7e]" : "text-[#969288]"
            )}
          >
            {label}
            {key === "pending" && !!pendingCount && (
              <span className="flex size-4 items-center justify-center rounded-full bg-[#52ad7e] text-[10px] text-[#fbf6e9]">
            ...
            {isActive && (
              <motion.div
                ...
                className="absolute inset-x-0 bottom-0 h-[2.5px] rounded-full bg-[#52ad7e]"
